...
const { mockSession, mockDb, mockTransaction, mockRoles } = vi.hoisted(() => {
  ...
  const mockTransaction = vi.fn().mockImplementation(async (cb: any) => {
    const tx = {
      usuarios: { create: vi.fn().mockResolvedValue({ id: 10, tenant_id: 1, nombre: 'New User', email: 'new@test.com' }) },
      usuarios_roles: { createMany: vi.fn() },
      areas: { update: vi.fn() },
    };
    return cb(tx);
...
describe('GET /api/usuarios', () => {
  ...
  it('returns paginated users', async () => {
    const users = [{ id: 1, nombre: 'Test', email: 'test@test.com', password_hash: 'secret', area: { id: 1, nombre: 'A' }, usuarios_roles: [] }];
...
describe('POST /api/usuarios', () => {
  ...
  it('creates a user', async () => {
    ...
    const req = new Request('http://localhost/api/usuarios', {
      method: 'POST',
      body: JSON.stringify({
        nombre: 'New User', email: 'new@test.com', password: 'Password1!@',
        area_id: 1, roles: ['solicitante'],
      }),
