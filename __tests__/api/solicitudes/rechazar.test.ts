...
vi.mock('@/lib/prisma', () => ({ prisma: { areas: { findFirst: vi.fn().mockResolvedValue(null) } }, tenantPrisma: vi.fn(() => mockDb) }));
...
function makeReq(body: any) {
  return new Request('http://localhost/api/solicitudes/1/rechazar', { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } });
}
...
describe('POST /api/solicitudes/[id]/rechazar', () => {
  ...
  it('transitions validada → rechazada with motivo', async () => {
    ...
    const res = await POST(makeReq({ motivo: 'No hay presupuesto disponible' }), { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    expect(mockDb.solicitudes.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ estado: 'rechazada' }) }));
  });
