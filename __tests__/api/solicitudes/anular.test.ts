...
const { mockSession, mockDb } = vi.hoisted(() => {
  const mockSession = { userId: 1, tenantId: 1, roles: ['director'], nombre: 'Test', email: 'test@test.com', areaId: 1 };
  const mockDb = {
...
import { POST } from '@/app/api/solicitudes/[id]/anular/route';

function makeReq(body: any) {
  return new Request('http://localhost/api/solicitudes/1/anular', { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } });
}

describe('POST /api/solicitudes/[id]/anular', () => {
  beforeEach(() => { vi.clearAllMocks(); });

  it('transitions an anulable state to anulada', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({ id: 1, estado: 'enviada', solicitante_id: 1, area_id: 1, titulo: 'Test', numero: 'SC-2026-0001', updated_at: new Date(), solicitante: { id: 1, nombre: 'Test' } });
    mockDb.solicitudes.update.mockResolvedValue({});
    const res = await POST(makeReq({ motivo: 'Ya no necesitamos este pedido' }), { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.estado).toBe('anulada');
  });

  it('returns 400 from terminal state (cerrada)', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({ id: 1, estado: 'cerrada', solicitante_id: 1, area_id: 1, titulo: 'Test', numero: 'SC-2026-0001', updated_at: new Date(), solicitante: { id: 1, nombre: 'Test' } });
    const res = await POST(makeReq({ motivo: 'Ya no necesitamos este pedido' }), { params: Promise.resolve({ id: '1' }) });
