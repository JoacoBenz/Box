...
function makeReq(body: any) {
  return new Request('http://localhost/api/solicitudes/1/anular', {
    method: 'POST',
...
describe('POST /api/solicitudes/[id]/anular', () => {
  ...
  it('transitions an anulable state to anulada', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({
      ...
      numero: 'SC-2026-0001',
      updated_at: new Date(),
      solicitante: { id: 1, nombre: 'Test' },
    });
    mockDb.solicitudes.update.mockResolvedValue({});
    const res = await POST(makeReq({ motivo: 'Ya no necesitamos este pedido' }), {
