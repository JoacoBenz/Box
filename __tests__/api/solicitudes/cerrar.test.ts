...
const { mockSession, mockDb } = vi.hoisted(() => {
  const mockSession = { userId: 1, tenantId: 1, roles: ['tesoreria'], nombre: 'Tesorero', email: 'tes@test.com', areaId: 1 };
  const mockDb = {
...
function makeReq(body: any = {}) {
  return new Request('http://localhost/api/solicitudes/1/cerrar', { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } });
}
...
describe('POST /api/solicitudes/[id]/cerrar', () => {
  ...
  it('transitions recibida_con_obs → cerrada', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({ id: 1, estado: 'recibida_con_obs', solicitante_id: 2, area_id: 1, titulo: 'Test', updated_at: new Date() });
    mockDb.solicitudes.update.mockResolvedValue({});
    const res = await POST(makeReq({ resolucion: 'Se reemplazó el item' }), { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    expect(mockDb.solicitudes.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ estado: 'cerrada' }) }));
  });
