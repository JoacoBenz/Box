...
const { mockSession, mockDb } = vi.hoisted(() => {
  const mockSession = { userId: 1, tenantId: 1, roles: ['responsable_area'], nombre: 'Test User', email: 'test@test.com', areaId: 1 };
  const mockDb = {
    solicitudes: { findFirst: vi.fn(), findMany: vi.fn(), update: vi.fn(), create: vi.fn(), count: vi.fn() },
  };
...
describe('POST /api/solicitudes/[id]/validar', () => {
  ...
  it('transitions enviada → validada', async () => {
    ...
    const req = new Request('http://localhost/api/solicitudes/1/validar', { method: 'POST', body: JSON.stringify({}), headers: { 'Content-Type': 'application/json' } });
    const res = await POST(req, { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.message).toBe('Solicitud validada');
    expect(mockDb.solicitudes.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ estado: 'validada' }) }));
  });

  it('returns 404 when solicitud not found', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue(null);
    const req = new Request('http://localhost/api/solicitudes/999/validar', { method: 'POST', body: JSON.stringify({}), headers: { 'Content-Type': 'application/json' } });
    const res = await POST(req, { params: Promise.resolve({ id: '999' }) });
    expect(res.status).toBe(404);
  });

  it('returns 400 for wrong state', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({ id: 1, estado: 'borrador', solicitante_id: 2, area_id: 1, titulo: 'Test', updated_at: new Date() });
    const req = new Request('http://localhost/api/solicitudes/1/validar', { method: 'POST', body: JSON.stringify({}), headers: { 'Content-Type': 'application/json' } });
    const res = await POST(req, { params: Promise.resolve({ id: '1' }) });
  ...
  it('returns 403 on segregation failure', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({ id: 1, estado: 'enviada', solicitante_id: 2, area_id: 1, titulo: 'Test', updated_at: new Date() });
    vi.mocked(verificarSegregacion).mockReturnValueOnce({ permitido: false, motivo: 'No podés validar tu propia solicitud' });
    const req = new Request('http://localhost/api/solicitudes/1/validar', { method: 'POST', body: JSON.stringify({}), headers: { 'Content-Type': 'application/json' } });
    const res = await POST(req, { params: Promise.resolve({ id: '1' }) });
