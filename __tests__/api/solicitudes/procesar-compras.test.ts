...
function makeReq(body: any) {
  return new Request('http://localhost/api/solicitudes/1/procesar-compras', { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } });
}
...
describe('POST /api/solicitudes/[id]/procesar-compras', () => {
  ...
  it('transitions en_compras → pago_programado', async () => {
    mockDb.solicitudes.findFirst.mockResolvedValue({ id: 1, estado: 'en_compras', solicitante_id: 2, area_id: 1, titulo: 'Test', updated_at: new Date() });
    mockDb.solicitudes.update.mockResolvedValue({});
    const res = await POST(makeReq({ prioridad_compra: 'normal', dia_pago_programado: futureDateStr }), { params: Promise.resolve({ id: '1' }) });
    expect(res.status).toBe(200);
    expect(mockDb.solicitudes.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ estado: 'pago_programado' }) }));
  });
