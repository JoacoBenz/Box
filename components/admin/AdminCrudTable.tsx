...
import { App, Table, Button, Modal, Form, Space, Skeleton, Popconfirm, Typography } from 'antd';
...
export interface AdminCrudTableProps<T extends { id: number }> {
  ...
  columns: (
    ...
    helpers: { openEdit: (item: T) => void; handleDeactivate: (item: T) => void },
  ) => ColumnsType<T>;
    ...
    <div className="page-content">
      ...
      {isMobile && renderMobileCard ? (
        loading ? (
          ...
          items.map((item) => (
            <div key={item.id}>{renderMobileCard(item, { openEdit, handleDeactivate })}</div>
          ))
        )
      ) : (
        <Table
