...
export function useAdminTenant(): [number | null, (id: number | null) => void] {
  ...
  const set = useCallback(async (id: number | null) => {
    setTenantId(id)
    try {
      await fetch('/api/admin/tenant-override', {
        method: 'POST',
...
export default function TenantSelector({ value, onChange, compact }: Props) {
  ...
  if (compact) {
    return (
      <Space size={6}>
        ...
        />
        {value && (
          <Tag color="purple" style={{ margin: 0, fontSize: 11 }}>Vista externa</Tag>
        )}
      </Space>
    )
  }
  ...
  return (
    <Space size={8} style={{
      ...
      />
      {value && (
        <Tag color="purple" style={{ margin: 0 }}>Vista externa</Tag>
      )}
    </Space>
  )
}
