...
export const POST = withAuth(
  ...
  async (request, { session, db, ip }, params) => {
    ...
    await db.solicitudes.update({
      ...
      data: {
        estado: 'rechazada',
        rechazado_por_id: session.userId,
        fecha_rechazo: new Date(),
        motivo_rechazo: validation.data.motivo,
      },
    });
    ...
    await crearNotificacion({
      ...
      mensaje: `${session.nombre} rechazó: ${solicitud.titulo}. Motivo: ${validation.data.motivo}`,
      solicitudId,
    });
    ...
    await registrarAuditoria({
      ...
      datosNuevos: { motivo: validation.data.motivo },
      ipAddress: ip,
    });
