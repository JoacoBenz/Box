...
export const POST = withAuth({}, async (request, { session, db, ip }, params) => {
  ...
  if (origen === 'responsable') {
    ...
    await db.solicitudes.update({ where: { id: solicitudId }, data: { estado: 'devuelta_resp', observaciones_responsable: validation.data.observaciones } });
    await crearNotificacion({ tenantId: session.tenantId, destinatarioId: solicitud.solicitante_id, tipo: 'solicitud_devuelta', titulo: 'Solicitud devuelta para corrección', mensaje: `${session.nombre}: ${validation.data.observaciones}`, solicitudId });
    ...
    await db.solicitudes.update({ where: { id: solicitudId }, data: { estado: 'devuelta_dir', observaciones_director: validation.data.observaciones } });

    const area = await prisma.areas.findFirst({ where: { id: solicitud.area_id, tenant_id: session.tenantId } });
    if (area?.responsable_id) {
      await crearNotificacion({ tenantId: session.tenantId, destinatarioId: area.responsable_id, tipo: 'solicitud_devuelta_dir', titulo: 'Solicitud devuelta por Dirección', mensaje: `${session.nombre}: ${validation.data.observaciones}`, solicitudId });
    }
    await crearNotificacion({ tenantId: session.tenantId, destinatarioId: solicitud.solicitante_id, tipo: 'solicitud_devuelta_dir', titulo: 'Solicitud devuelta por Dirección', mensaje: `${session.nombre}: ${validation.data.observaciones}`, solicitudId });
  } else {
    return apiError('BAD_REQUEST', 'origen debe ser responsable o director', 400);
  }

  await registrarAuditoria({ tenantId: session.tenantId, usuarioId: session.userId, accion: `devolver_${origen}`, entidad: 'solicitud', entidadId: solicitudId, datosNuevos: { observaciones: validation.data.observaciones }, ipAddress: ip });
