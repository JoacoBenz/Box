...
export const POST = withAuth(
  ...
  async (request, { session, db, ip }, params) => {
    ...
    await db.solicitudes.update({
      ...
      data: {
        estado: 'validada',
        validado_por_id: session.userId,
        fecha_validacion: new Date(),
        observaciones_responsable: observaciones,
      },
    });
    ...
    await notificarPorRol(
      session.tenantId,
      'director',
      'Solicitud lista para aprobar',
      `${session.nombre} validó: ${solicitud.titulo}`,
      solicitudId,
    );
    await crearNotificacion({
      tenantId: session.tenantId,
      destinatarioId: solicitud.solicitante_id,
      tipo: 'solicitud_validada',
      titulo: 'Tu solicitud fue validada',
      mensaje: `Tu solicitud "${solicitud.titulo}" pasó a aprobación de Dirección`,
      solicitudId,
    });
