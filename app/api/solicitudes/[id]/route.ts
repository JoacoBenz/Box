...
export const GET = withAdminOverride(
  {},
  async (request, { session, db, effectiveTenantId }, params) => {
    ...
    if (!tieneAcceso)
      return Response.json(
        { error: { code: 'FORBIDDEN', message: 'Sin acceso a esta solicitud' } },
        { status: 403 },
      );
...
export const PATCH = withAuth({}, async (request, { session, db, ip }, params) => {
    ...
    return Response.json(
      {
        error: {
          code: 'BAD_REQUEST',
          message: 'Solo se pueden editar solicitudes en borrador o devueltas',
        },
      },
      { status: 400 },
    );
  ...
  if (solicitud.solicitante_id !== session.userId) {
    return Response.json(
      { error: { code: 'FORBIDDEN', message: 'Solo el solicitante puede editar' } },
      { status: 403 },
    );
  }
    ...
    return Response.json(
      {
        error: {
          ...
          details: [{ field: 'items', message: 'Agregá al menos un ítem' }],
        },
      },
      { status: 400 },
    );
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (proveedor_sugerido !== undefined)
      updateData.proveedor_sugerido = proveedor_sugerido ?? null;
