...
export const GET = withAuth({ roles: ['director', 'compras', 'tesoreria', 'admin'] }, async (request, { session }) => {
  ...
  const [gastosPorProducto, gastosPorArea, evolucionMensual, topProveedores, productosMasSolicitados] = await Promise.all([
    prisma.$queryRaw<any[]>`
      SELECT
  ...
  const num = (v: any) => v != null && typeof v === 'object' && 'toNumber' in v ? (v as any).toNumber() : typeof v === 'bigint' ? Number(v) : (v ?? 0);

  const workbook = new ExcelJS.Workbook();

  // Sheet 1: Gasto por Producto
  const s1 = workbook.addWorksheet('Gasto por Producto');
  styleHeader(s1.addRow(['Producto', 'Área', 'Cantidad Total', 'Gasto Total', 'Solicitudes', 'Último Precio']));
  for (const r of gastosPorProducto) {
    s1.addRow([r.producto, r.area ?? '—', num(r.cantidad_total), num(r.gasto_total), num(r.num_solicitudes), num(r.ultimo_precio)]);
  }
  s1.getColumn(1).width = 30;
  s1.getColumn(2).width = 18;
  s1.getColumn(3).width = 16;
  s1.getColumn(4).width = 16; s1.getColumn(4).numFmt = '#,##0.00';
  s1.getColumn(5).width = 14;
  s1.getColumn(6).width = 16; s1.getColumn(6).numFmt = '#,##0.00';
  ...
  s2.getColumn(2).width = 16; s2.getColumn(2).numFmt = '#,##0.00';
  ...
  s3.getColumn(2).width = 16; s3.getColumn(2).numFmt = '#,##0.00';
  ...
  s5.getColumn(2).width = 16; s5.getColumn(2).numFmt = '#,##0.00';
