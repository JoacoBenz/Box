...
export const PATCH = withAdminOverride({ roles: ['admin', 'director'] }, async (request, { session, db, ip, effectiveTenantId }, params) => {
  ...
  if (typeof body.activo === 'boolean' && Object.keys(body).length === 1) {
    ...
    await registrarAuditoria({ tenantId: effectiveTenantId ?? session.tenantId, usuarioId: session.userId, accion: body.activo ? 'activar_area' : 'desactivar_area', entidad: 'area', entidadId: areaId, ipAddress: ip });
...
export const DELETE = withAdminOverride({ roles: ['admin', 'director'] }, async (request, { session, db, ip, effectiveTenantId }, params) => {
  ...
  await registrarAuditoria({ tenantId: effectiveTenantId ?? session.tenantId, usuarioId: session.userId, accion: 'desactivar_area', entidad: 'area', entidadId: areaId, ipAddress: ip });
