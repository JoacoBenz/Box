  ...
  async (request, { session, db, ip, effectiveTenantId }, params) => {
    ...
    if (typeof body.activo === 'boolean' && Object.keys(body).length === 1) {
      ...
      await registrarAuditoria({
        ...
        accion: body.activo ? 'activar_centro_costo' : 'desactivar_centro_costo',
    ...
    if (codigo) {
      ...
      const dup = await db.centros_costo.findFirst({
        where: { codigo: codigoUpper, id: { not: centroId } },
      });
      if (dup)
        return Response.json(
          {
    ...
    if (nombre) {
      const dup = await db.centros_costo.findFirst({
        where: { nombre: { equals: nombre }, activo: true, id: { not: centroId } },
      });
      if (dup)
        return Response.json(
          {
    ...
    if (targetAreaId) {
      ...
      if (area) {
        const existingCCs = await db.centros_costo.findMany({
          where: { area_id: targetAreaId, activo: true, id: { not: centroId } },
          select: { presupuesto_anual: true, presupuesto_mensual: true },
        });
        const newAnual =
          presupuesto_anual !== undefined
            ? Number(presupuesto_anual ?? 0)
            : Number(centro.presupuesto_anual ?? 0);
        const newMensual =
          presupuesto_mensual !== undefined
            ? Number(presupuesto_mensual ?? 0)
            : Number(centro.presupuesto_mensual ?? 0);
        const sumaAnual =
          existingCCs.reduce((s, cc) => s + Number(cc.presupuesto_anual ?? 0), 0) + newAnual;
        const sumaMensual =
          existingCCs.reduce((s, cc) => s + Number(cc.presupuesto_mensual ?? 0), 0) + newMensual;
