...
export const POST = withAuth({ roles: ['solicitante', 'responsable_area'] }, async (request, { session, db, ip }) => {
  ...
  await registrarAuditoria({ tenantId: session.tenantId, usuarioId: session.userId, accion: 'confirmar_recepcion', entidad: 'recepcion', entidadId: solicitud_id, datosNuevos: { conforme, tipo_problema }, ipAddress: ip });
