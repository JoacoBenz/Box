...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const area of areas) {
      ...
      if (
        areaNorm === areaTextoNorm ||
        areaNorm.includes(areaTextoNorm) ||
        areaTextoNorm.includes(areaNorm)
      ) {
    ...
    if (!solicitanteRole) {
      return Response.json(
        { error: { code: 'INTERNAL', message: 'Rol solicitante no encontrado' } },
        { status: 500 },
      );
    }
    ...
    await registrarAuditoria({
      ...
      datosNuevos: {
        ...
        email,
        area_texto,
        metodo: codigo ? 'codigo_invitacion' : 'dominio',
        verificado: false,
      },
