  ...
  async (request, { session, db, ip, effectiveTenantId }, params) => {
    ...
    if (isOnlyResponsable(session.roles)) {
      if (usuario.area_id !== session.areaId) {
        return Response.json(
          { error: { code: 'FORBIDDEN', message: 'Solo podés modificar usuarios de tu área' } },
          { status: 403 },
        );
      }
        ...
        return Response.json(
          {
            error: {
              code: 'FORBIDDEN',
              message: 'Solo podés modificar usuarios con rol solicitante',
            },
          },
          { status: 403 },
        );
      ...
      return Response.json(
        {
          error: {
            code: 'FORBIDDEN',
            message: 'Solo un administrador de plataforma puede modificar usuarios admin',
          },
        },
        { status: 403 },
      );
    ...
    if (!nuevoActivo) {
      if (targetIsAdmin) {
        ...
        if (adminCount <= 1) {
          return Response.json(
            { error: { code: 'CONFLICT', message: 'No podés desactivar al único administrador' } },
            { status: 409 },
          );
        }
      }
