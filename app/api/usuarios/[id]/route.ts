...
export const PATCH = withAdminOverride(
  ...
  async (request, { session, db, ip, effectiveTenantId }, params) => {
    ...
    if (isOnlyResponsable(session.roles)) {
      if (!session.areaId) {
        return Response.json(
          { error: { code: 'FORBIDDEN', message: 'No tenés un área asignada' } },
          { status: 403 },
        );
      }
      if (usuario.area_id !== session.areaId) {
        return Response.json(
          { error: { code: 'FORBIDDEN', message: 'Solo podés editar usuarios de tu área' } },
          { status: 403 },
        );
      }
      ...
      if (targetRoles.some((r) => r !== 'solicitante')) {
        return Response.json(
          {
            error: { code: 'FORBIDDEN', message: 'Solo podés editar usuarios con rol solicitante' },
          },
          { status: 403 },
        );
      }
      if (roleNames.length !== 1 || roleNames[0] !== 'solicitante') {
        return Response.json(
          { error: { code: 'FORBIDDEN', message: 'Solo podés asignar el rol solicitante' } },
          { status: 403 },
        );
      }
    ...
    if (!isAdmin) {
        ...
        return Response.json(
          {
            error: {
              code: 'FORBIDDEN',
              message: 'Solo un administrador de plataforma puede asignar el rol admin',
            },
          },
          { status: 403 },
        );
      ...
      if (targetHasAdmin) {
        return Response.json(
          { error: { code: 'FORBIDDEN', message: 'No podés editar un usuario con rol admin' } },
          { status: 403 },
        );
      }
    }
