...
export const POST = withAdminOverride(
  { roles: ['admin', 'director', 'responsable_area'] },
  async (request, { session, effectiveTenantId, ip }) => {
    ...
    if (existingEmail)
      return Response.json(
        { error: { code: 'CONFLICT', message: 'Email ya registrado en esta organización' } },
        { status: 409 },
      );
    ...
    if (!area)
      return Response.json(
        { error: { code: 'NOT_FOUND', message: 'Área no encontrada o inactiva' } },
        { status: 404 },
      );
    ...
    await registrarAuditoria({
      ...
      datosNuevos: { nombre, email, roles: roleNames },
      ipAddress: ip,
    });
