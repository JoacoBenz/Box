  ...
  try {
    ...
    if (body.estado && ESTADOS_VALIDOS.includes(body.estado)) {
      if (tenantId === session.tenantId) {
        return apiError(
          'VALIDATION_ERROR',
          'No podés cambiar el estado de tu propia organización',
          400,
        );
      }
...
export async function DELETE(
  ...
  try {
    ...
    if (solicitudes > 0 || compras > 0 || proveedores > 0) {
      return apiError(
        'VALIDATION',
        'No se puede eliminar: la organización tiene solicitudes, compras o proveedores. Desactivala en su lugar.',
        400,
      );
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const userIds = (
        await tx.usuarios.findMany({ where: { tenant_id: tenantId }, select: { id: true } })
      ).map((u) => u.id);
      if (userIds.length > 0) {
