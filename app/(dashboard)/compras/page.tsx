    ...
    try {
      ...
      if (res.ok) {
        ...
        const sorted = (data.data ?? []).sort((a: Solicitud, b: Solicitud) => {
          const dateA = a.dia_pago_programado
            ? new Date(a.dia_pago_programado).getTime()
            : Infinity;
          const dateB = b.dia_pago_programado
            ? new Date(b.dia_pago_programado).getTime()
            : Infinity;
          return dateA - dateB;
        });
  ...
  return (
    <div className="page-content">
      ...
      <p style={{ color: 'var(--text-muted)', marginBottom: 16 }}>
        Solicitudes con pago programado, ordenadas por fecha de pago. El botón se habilita cuando
        llega la fecha.
      </p>
      <Table
        rowKey="id"
