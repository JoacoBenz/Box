...
export default function RegistrarCompraForm({ solicitud, archivos = [] }: Props) {
  ...
  const [proveedorData, setProveedorData] = useState<ProveedorData | null | undefined>(
    solicitud.proveedor,
  );
  ...
  useEffect(() => {
    if (prov) {
      const detalle = [
        prov.cuit && `CUIT: ${prov.cuit}`,
        prov.datos_bancarios,
        prov.direccion && `Dir: ${prov.direccion}`,
  ...
  async function handleSubmit() {
    try {
      ...
      if (values.referencia_bancaria)
        formData.append('referencia_bancaria', values.referencia_bancaria);
  ...
  async function handleSaveBancarios() {
    ...
    try {
      ...
      setProveedorData((prev) =>
        prev
          ? { ...prev, datos_bancarios: updated.datos_bancarios ?? values.datos_bancarios }
          : prev,
      );
      // Also update the proveedor_detalle form field
      const detalle = [
        prov.cuit && `CUIT: ${prov.cuit}`,
        values.datos_bancarios,
        prov.direccion && `Dir: ${prov.direccion}`,
        prov.telefono && `Tel: ${prov.telefono}`,
      ]
        .filter(Boolean)
  ...
  return (
    <div className="page-content" style={{ maxWidth: 880, margin: '0 auto' }}>
      ...
      {/* Solicitud summary */}
      <Card
        title={<span style={{ fontWeight: 700, color: tokens.textPrimary }}>Solicitud</span>}
        ...
        <Descriptions column={2} size="small">
          ...
          {pagoDate && (
            <Descriptions.Item label="Fecha de Pago Programado">
              <Tag color={canSubmit ? 'green' : 'orange'}>{pagoDate.format('DD/MM/YYYY')}</Tag>
              {!canSubmit && (
                <span style={{ color: 'var(--color-observation)', fontSize: 12, marginLeft: 8 }}>
                  Aún no habilitado
                </span>
              )}
            </Descriptions.Item>
      ...
      {prov && (
        <ProveedorInfoCard
          ...
          editable
          onEditBancarios={openEditBancarios}
        />
      )}
          ...
          {archivos.map((a) => (
            ...
            >
              ...
              <a
                ...
                style={{ color: tokens.colorPrimary, fontWeight: 600, fontSize: 13 }}
              >
                Descargar
              </a>
            </div>
      ...
      <Card
        title={
          <span style={{ fontWeight: 700, color: tokens.textPrimary }}>Datos de la Compra</span>
        }
      ...
      >
        <Form form={form} layout="vertical" {...formProps}>
          ...
          <Form.Item label="Detalle del Proveedor" name="proveedor_detalle">
            <TextArea
              rows={2}
              placeholder="CUIT, dirección, teléfono, etc."
              maxLength={500}
              readOnly={!!prov}
              disabled={!!prov}
            />
          ...
          <Space style={{ width: '100%' }} size={24} wrap>
            ...
            <Form.Item
              ...
              rules={[
                ...
                ...(() => {
                  const totalEstimado = (solicitud.items_solicitud ?? []).reduce(
                          ...
                          validator: (_: any, value: number) => {
                            ...
                            if (value > totalEstimado * 3)
                              return Promise.reject(
                                `El monto ($${value.toFixed(2)}) supera 3x el estimado ($${totalEstimado.toFixed(2)}). Verificá si es correcto.`,
                              );
                            return Promise.resolve();
                          },
                        },
            ...
            {(medioPago === 'transferencia' || medioPago === 'cheque') && (
              <Form.Item
                ...
                rules={[
                  {
                    required: true,
                    message: 'La referencia bancaria es obligatoria para este medio de pago',
                  },
                ]}
              >
          ...
          <Form.Item label="Comprobante (PDF, JPG o PNG — máx. 10 MB)" required>
            <Upload
              beforeUpload={(file) => {
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            >
              {canSubmit
                ? 'Registrar Pago'
                : `Habilitado el ${pagoDate?.format('DD/MM/YYYY') ?? ''}`}
            </AnimatedSubmitButton>
