    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        {!isUrgent && (
          <Checkbox
            checked={selected}
            onChange={(e) => onSelect(solicitud.id, e.target.checked)}
          />
        )}
        <Text
          strong
        ...
        {u && <Tag color={u.color} style={{ margin: 0, fontSize: 11 }}>{u.label}</Tag>}
      </div>
      ...
      {/* Info row */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 16px', fontSize: 12, color: 'var(--text-secondary)', marginBottom: 10 }}>
...
export default function AprobacionesPage() {
  ...
  return (
    <div className="page-content">
      <Title level={3} style={{ margin: 0, marginBottom: 4, fontWeight: 700, color: 'var(--text-primary)' }}>
        ...
        <div>
          {loading ? (
            Array.from({ length: 3 }).map((_, i) => (
              <div key={i} style={{ background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border-color)', padding: 14, marginBottom: 10 }}>
