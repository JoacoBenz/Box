...
async function getRequiredApprovalLevel(
  ...
  const umbralResponsable = await getTenantConfigNumber(tenantId, 'umbral_aprobacion_responsable', 0);
  const umbralDirector = await getTenantConfigNumber(tenantId, 'umbral_aprobacion_director', 999_999_999);
...
export async function canUserApproveAmount(
  ...
  switch (requiredLevel) {
    case 'responsable':
      ...
      return { allowed: false, reason: 'Se requiere al menos un responsable de área para este monto', requiredLevel };

    case 'director':
      if (userRoles.some(r => ['director', 'admin'].includes(r))) {
        return { allowed: true, requiredLevel };
      }
      return { allowed: false, reason: 'Se requiere aprobación de un director para este monto', requiredLevel };

    case 'director_general':
      if (userRoles.includes('admin')) {
        return { allowed: true, requiredLevel };
      }
      return { allowed: false, reason: 'Este monto excede el límite de aprobación del director. Se requiere aprobación de administración.', requiredLevel };
  }
}
