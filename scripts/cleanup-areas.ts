...
async function main() {
  ...
  for (const t of tenants) {
    ...
    for (const a of otherAreas) {
      await prisma.usuarios.updateMany({ where: { area_id: a.id }, data: { area_id: direccion.id } });
    ...
    if (director) {
      await prisma.areas.update({ where: { id: direccion.id }, data: { responsable_id: director.id } });
    }
  }
